  ...
  define("Required", (v) => {
    ...
    if (valid) {
      if (isObject(v) || isArray(v) || isSet(v)) {
        ...
        if (!Object.is(NaN, Number(v))) valid = v.length > 0;
        else valid = true;
      }
    }
    return valid || error;
...
export const ValueIn = (
  target = [],
  error = "value tidak ada di dalam requirements"
) =>
...
export const CustomFunc = (
  callback,
  error = "validation error using custom validation"
) => define("CustomFunc", (v) => callback(v) || error);

export const Between = (
  min,
  max,
  error = "value must be between in requirements"
) =>
  define("Between", (v) => {
    let valid = false;
    if (typeof v == "string") {
